...
```
tsto2rgb -a 1.0 -b path/to/building/ -o path/to/dlc_name/
```

You can specify a default delay with the --delay [-d] option to use with the bcell files. The delay can be a decimal number and must be specified in milliseconds terms.

```
tsto2rgb -d 41.68 -c path/to/character/ -o path/to/dlc_name/
```
