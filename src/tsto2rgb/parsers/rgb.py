...
def rgb_parser(img_file, target, depth):

    with Image(filename=img_file) as main_img:
        if main_img.alpha_channel is False:
            main_img.alpha_channel = "opaque"
        if depth == 4:
            with main_img.fx("u * a", channel="rgb") as tmp_img:
                width = tmp_img.width
...
def rgb_gen(files, target, total, input_extension, depth):
    ...
    for i in range(total):
        status = rgb_parser(files[i], Path(target, files[i].stem + ".rgb"), depth)
        report_progress(
